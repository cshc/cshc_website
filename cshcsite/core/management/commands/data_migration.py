...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        ...
        #    dest='convert',
        #    default=False,
        #    help='Run MDBtoCSV.exe converter'),
        )
    ...
    def _handle(self, *args, **options):
        ...
        #######################################################################
        # CONVERT MDB to CSV - deprecated

        #if options['convert']:
        #    converter_exe = os.path.join('.', IMPORT_DIR, 'MDBtoCSV.exe')
        #    mdb_path = os.path.join('.', IMPORT_DIR, MDB_FILE)
        #    output_dir = os.path.join('.', IMPORT_DIR, CONVERSION_OUTPUT_DIR)
        #    if not os.path.exists(output_dir):
        #        os.mkdir(output_dir)
        #    rtn = subprocess.call([converter_exe, mdb_path, output_dir + os.sep])
        #    if rtn != 0:
        #        print "ERROR: Failed to convert MDB file"
        #        return
        ...
        for table_name, old_table in old_tables.iteritems():
            ...
            with open(file_path, 'rb') as table_file:
                # Old method - using MDBtoCSV.exe output

                #content = table_file.read()
                #content = Command.sanitise(content)
            
                #lines = content.split(';\r\n')
                #for line in lines[:-1]:
                #    field_values = line.split(';')
                #    row = old_table.table_class(field_values)
                #    old_table.rows[row.pk] = row
                #    #print "Imported player: {0}".format(p)
