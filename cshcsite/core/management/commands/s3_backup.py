...
BACKUP_FILENAME = '/home/ubuntu/prod_dump.mysql'
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        try:
            ...
            print "1. Downloading latest MySQL database backup from S3"
            ...
            print "2. Unzipping MySQL database backup to {}".format(BACKUP_FILENAME)
            with gzip.open(GZ_BACKUP_FILENAME, 'rb') as in_file:
                with open(BACKUP_FILENAME, 'wb') as out_file:
                    out_file.write(in_file.read())

            print "3. Copy all keys (files) from production bucket's media directory to corresponding staging bucket's media directory"
