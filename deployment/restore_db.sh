#!/bin/bash
#
# WARNING - THIS WORKS ON THE PRODUCTION DATABASE! USE WITH EXTREME CAUTION!!
#
# Usage: revert_db.sh <dump.sql>
#
# where <dump.sql> is the mysqldump output file to restore the DB from.
#
# Note - this assumes the MySQL username and password are stored in the .my.cnf
# file
...
if [[ $REPLY =~ ^[Yy]$ ]]
then
    MYSQL="mysql -h mysql-51.int.mythic-beasts.com -D cshc"
